...
class Initiate(object):
    ...
    def __init__(self, master):
        ...
        self.menu.add_command(label = 'Move: Completed', command = lambda: self.move_to_completed())
...
##        self.master.bind('<3>', lambda: self.menu.post(x, y))
    ...
    def move_to_inprogress(self):
        """
        for lbs in self.listboxhold.values():
            if foc == lbs:
                print('match')
            else:
                print('no match')
        ...
        foc = self.master.focus_get()
        foc_lbox = [i for i in self.listboxhold.values() if i == foc]
        try:
            
                att = foc_lbox[0].curselection()        #Grab selected items from listbox
                print(len(att))
                
                for i in att:
                    cur_ent = foc_lbox[0].get(i)
        ...
        foc_lbox[0].selection_clear(0, tk.END)
        foc_lbox[0].selection_set(0)
        self.update_totals_labels()
